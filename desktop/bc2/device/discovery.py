...
def sign_transaction_input(
    port_name, plan, position, hash_prevouts, hash_sequence, change_address
):
    with serial.Serial(
        port_name,
        baudrate=DeviceDiscovery.BAUD_RATE,
        ...
        return BC2DeviceClient(port).sign_transaction_input(
            plan, position, hash_prevouts, hash_sequence, change_address
        )
