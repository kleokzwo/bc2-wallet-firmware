...
int main(void) {
    ...
    assert(bc2_pin_security_create(&security, &hal, "246813") == BC2_PIN_SECURITY_OK);
    assert(storage.record_size > BC2_SECURITY_PIN_LENGTH);
    assert(!contains_pin(storage.record, storage.record_size, "246813"));
    assert(bc2_pin_security_verify(&security, &hal, "246813", 0U) == BC2_PIN_SECURITY_OK);
    assert(bc2_pin_security_verify(&security, &hal, "111111", 100U) == BC2_PIN_SECURITY_INVALID);
    assert(bc2_pin_security_verify(&security, &hal, "222222", 200U) == BC2_PIN_SECURITY_INVALID);
    assert(bc2_pin_security_verify(&security, &hal, "333333", 300U) == BC2_PIN_SECURITY_INVALID);
    assert(bc2_pin_security_remaining_delay(&security, 300U) == 5000U);
    assert(bc2_pin_security_verify(&security, &hal, "246813", 400U) == BC2_PIN_SECURITY_DELAYED);
    assert(bc2_pin_security_verify(&security, &hal, "246813", 5300U) == BC2_PIN_SECURITY_OK);
